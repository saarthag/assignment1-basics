...
from cs336_basics import transformer, training_utils
...
def run_embedding(
    ...
    e.load_state_dict({"weight": weights}, strict=True)
...
def run_transformer_lm(
    ...
    return lm(token_ids=in_indices)
...
def run_rmsnorm(
    ...
    r.load_state_dict({"weight": weights}, strict=True)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    return transformer.my_softmax(x=in_features, dim=dim)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    transformer.clip_grad(parameters=parameters, max_l2_norm=max_l2_norm)
...
def run_get_lr_cosine_schedule(
    ...
    """
    return transformer.cosine_lr_schedule(
        it=it,
...
def run_save_checkpoint(
    ...
    """
    training_utils.save_checkpoint(model=model, optimizer=optimizer, iteration=iteration, out=out)
...
def run_load_checkpoint(
    ...
    """
    return training_utils.load_checkpoint(src=src, model=model, optimizer=optimizer)
...
def get_tokenizer(
    ...
    """
    return bpe.BPETokenizer(vocab=vocab, merges=merges, special_tokens=special_tokens)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return bpe.train_bpe_fast(input_path=input_path, vocab_size=vocab_size, special_tokens=special_tokens)
